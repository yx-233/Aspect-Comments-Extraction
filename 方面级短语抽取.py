...
# 定义输入参数
data = pd.read_excel("data/sorce/一级公共图书馆.xlsx")
...
df = pd.DataFrame({"comments":S_prime})
df.to_excel(f"data/方面级：{T[0]}.xlsx",index=False)
df_list = pd.DataFrame({"comments":save_list})
df_list.to_excel("data/方面级短语.xlsx",index=False)
# print(S_prime)
